 ...
 * @api public
 */
exports.findArticleById = function(req, res, callback) {
  var id = req.query['id'];
 ...
 * @api public
 */
exports.findPageById = function(req, res, callback) {
  var id = req.query['id'];
 ...
 * @api public
 */
exports.findArticleByDate = function(req, res, callback) {
  var p = req.params;
  var start = new Date(p.year, p.month-1, p.day);   //months are 0 based so Jan equals 0
  var end = new Date(p.year, p.month-1, p.day);
  Article.findOne({ts: {$gte: start, $lte: end}, alias: p.title}, function(err, doc) {
    if (err) {
      util.log('FATAL '+ err);
      callback(err, null, req, res);
    }
