...
exports.projectPreferences = function(params, callback){
  Prefs.findOne({ }, function(err, doc) {
    ...
    if (doc===null){
      doc = { items: params.config.per_page, description: "", keywords: "" };
    }            
    callback(null, doc);
  });
}
exports.save = function(items, description, keywords, callback) {
  Prefs.findOne({ }, function(err, doc) {
    if (err) {
        util.log('FATAL '+ err);
        callback(err);
      ...
      if (doc===null){
          exports.newPref(items, description, keywords, callback);
      } else {
        doc.items = items;
        doc.description = description;
        doc.keywords = keywords;
